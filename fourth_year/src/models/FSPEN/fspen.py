...
class FullSubPathExtension(nn.Module):
    def __init__(self, configs):
        ...
        self.ch_size=configs['channel_size']
        ...
        self.complex_multi_to_single = nn.Linear(in_features=self.ch_size, out_features=1)
        self.complex_multi_to_single.weight.data.fill_(0.5)

        self.amp_multi_to_single = nn.Linear(in_features=self.ch_size, out_features=1)
        self.amp_multi_to_single.weight.data.fill_(0.5)
    ...
    def forward(self, in_complex_spectrum: Tensor, in_amplitude_spectrum: Tensor, hidden_state: list):
        ...
        :return:
        """
        in_complex_spectrum_1ch=in_complex_spectrum[:,0]
        in_amplitude_spectrum_1ch=in_amplitude_spectrum[:,0]
        in_complex_spectrum = in_complex_spectrum.permute(0, 2,3,4,1)
        in_complex_spectrum=self.complex_multi_to_single(in_complex_spectrum)[...,0]
        in_amplitude_spectrum = in_amplitude_spectrum.permute(0, 2,3,4,1)
        in_amplitude_spectrum=self.amp_multi_to_single(in_amplitude_spectrum)[...,0]
        ...
        full_band_out=in_complex_spectrum_1ch*full_band_mask
        sub_band_out=in_amplitude_spectrum_1ch*sub_band_mask
