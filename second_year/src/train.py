...
class Learner_config():
    ...
    def init_loss_func(self):
        ...
        elif self.args['learner']['loss']['type']=='SI-SDR':
...
class Trainer():
    ...
    def train(self, epoch):
        ...
        for iter_num, (mixed, target) in enumerate(tqdm(self.dataloader.train_loader, desc='Train {}'.format(epoch), total=len(self.dataloader.train_loader), )):

            mixed=mixed.to(self.hyperparameter.device)
            target=target.to(self.hyperparameter.device)
            ...
            self.learner.memory_delete([mixed, out, target, loss])
    ...
    def test(self, epoch):
        self.model.eval()
        with torch.no_grad():
            for iter_num, (mixed, target) in enumerate(tqdm(self.dataloader.test_loader, desc='Test', total=len(self.dataloader.test_loader), )):
