...
class ConvSTFT(nn.Module):
    ...
    def azimuth_strided(self, vad, azi):
        ...
        return azi
        azi=torch.round(torch.stack(result, dim=-1)).unsqueeze(-1)

        azi_range=torch.arange(0, 360).unsqueeze(0).to(azi.device).repeat_interleave(spk_num,dim=0).unsqueeze(1)

        distance=azi-azi_range

        print(azi_range.shape, azi.shape, distance.shape)
        exit()
