...
class TCN(nn.Module):
    def __init__(self, ch_size, input_dim, output_dim, BN_dim, hidden_dim,
                 layer, stack, kernel=3, skip=True, 
                 causal=False, dilated=True, Film_loc=8):
        ...
        # normalization
        if not causal:
            self.LN = nn.GroupNorm(1, input_dim*ch_size, eps=1e-8)
        else:
            self.LN = cLN(input_dim*ch_size, eps=1e-8)

        self.BN = nn.Conv1d(input_dim*ch_size, BN_dim, 1)
    ...
    def forward(self, input, ssl_weight, ssl_bias):
