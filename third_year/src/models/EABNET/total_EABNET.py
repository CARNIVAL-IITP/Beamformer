# originally from https://github.com/Andong-Li-speech/EaBNet
import numpy as np
import torch
...
class Total_model(nn.Module):
    ...
    def forward(self, input_wav):
        ...
        input_stft=torch.stack((input_stft_r, input_stft_i), dim=-1).permute(0, 3, 2,1, 4)
        
        beamforming_weight=self.model(input_stft)
        
        input_stft_complex=torch.view_as_complex(input_stft)

        out_stft=input_stft_complex*beamforming_weight
        out_stft=out_stft.sum(dim=-1).permute(0,2,1)
        out_wav=self.iSTFT(out_stft.real, out_stft.imag, cplx=True)
        out_wav=out_wav[..., :original_len]

        beamforming_weight_imag=beamforming_weight.imag
        beamforming_weight_real=beamforming_weight.real
        
        return out_wav, beamforming_weight_real, beamforming_weight_imag
...
class EaBNet(nn.Module):
    ...
    def forward(self, inpt: Tensor) -> Tensor:
        ...
        if self.topo_type == "mimo":
            ...
            bf_w=torch.view_as_complex(bf_w)
            return bf_w
